...
def Instructions():
    Instruction = argparse.ArgumentParser(description="For job submission on LSF/BSUB system \nUsage 101:\nfor i in {1..10}; do echo 'echo '${i} >> do.sh\nsbatch.py -s do.sh -n test -p low -c 1 -t 1")
...
if __name__ == '__main__':
    ...
    if Instructions().yes == 'yes':
        ...
        with open(historyFile, 'a') as f:
            f.write(cmd + '\n')
        l1 = '#!/bin/bash'
        l2 = '#BSUB -J ' + Instructions().jobName
        l3 = '#BSUB -q ' + Instructions().partition
        l4 = '#BSUB -n ' + str(Instructions().cpuNumber)
        # l5 = '#SBATCH --ntasks-per-node=' + str(Instructions().threadsPerCPU)
        l6 = '#BSUB -e ' + Instructions().jobName + '.err'
        l7 = '#BSUB -o ' + Instructions().jobName + '.out'
        # l8 = 'CURDIR=`pwd`'
        l9 = 'cd ' + os.path.abspath('./') + '/'
        l10 = '\n'.join(openfile(Instructions().scriptName, nohash='True'))
        all = '\n'.join([l1, l2, l3, l4, l6, l7, l9, l10])
        print('Sbatch Submission is \n' + all)
        with open('./' + Instructions().jobName + '.sh', 'w') as f:
            f.write('\n'.join([l1, l2, l3, l4, l6, l7, l9, l10]) + '\n')
        do = os.popen('bsub < ' + Instructions().jobName + '.sh')
        print('Submission is done!')
    elif Instructions().yes == 'no':
        l1 = '#!/bin/bash'
        l2 = '#BSUB -J '+Instructions().jobName
        l3 = '#BSUB -q '+Instructions().partition
        l4 = '#BSUB -n '+str(Instructions().cpuNumber)
