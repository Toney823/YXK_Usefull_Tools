...
def block_cacherGff(lines: list):
    file_blocks = []
    block = []
    for line in lines:
        if len(line) == 0:
...
for b in block_cacherGff(openGff(og)):
    if len(b) == 0:
        continue
    chr = b[0].strip().split()[0]
    if chr in dick:
       dick[chr].append(b[:])
